		...
		<div class="web-box dk cart-item" v-for="(item, i) in this.items" :id="'cartItem' + item.id">
			...
			<div class="cart-item-content">
				<div class="cart-item-top-row">
					<a :href="'/product-page/' + item.productId">
						<h3>{{ item.title }}</h3>
					</a>
				...
				<div class="form cart-item-bottom-row">
					...
					<input type="number" min="1" name="quantity" :id="'quantity' + item.id" v-model="item.quantity"
						v-on:change="quantityChange(item.id, $event.target.value, item.price)">
