<template>
  <nav class="mobile-nav lt">
    <a href="/" class="title">
      <h3 class="hover">{{ this.sitetitle }}</h3>
    </a>
    ...
    <div v-if="this.sessionuser" class="nav-link">
      ...
      <ul :class="'sublist' + 999" class="nav-sublinks">
        <a v-for="(sublink, i) in this.userlinks" :href="sublink['link']" class="nav-sublink hover-background">
    ...
    <ul>
      <div v-for="(link, i) in this.publiclinks" class="nav-link">
        ...
        <a v-if="link['sublink']" class="primary-nav-link">
          <i :class="[link['icon']]" class="nav-link-icon"></i>
          <li>{{ capFL(link['title']) }}</li>
          <i @click="toggleLinks(i, false)" v-show="link['sublink']" class="fa-solid fa-angle-down hover-background" :class="'ladown' + [i]"></i>
          <i @click="toggleLinks(i, true)" v-show="link['sublink']" class="fa-solid fa-angle-up hover-background" :class="'laup' + [i]"></i>
        </a>
